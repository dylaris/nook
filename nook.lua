...
-- short_name: like "-h"
-- long_name: like "--help"
-- config: includes nargs, default, desc, required
...
parser:add("-h", "--help",   { nargs = 0, desc = "show this help message" })
...
parser:add("-l", "--list",   { nargs = 0, desc = "list all entries" })
parser:add("-t", "--format", { nargs = 1, desc = "set output format type", default = "brief" })
parser:add("-f", "--filter", { nargs = 1, desc = "set filter type" })
...
for _, kind in ipairs(filter_opt.args) do
  local func = rule.filter[kind]
  if not func then
    print("error: filter '" .. kind .. "' is not defined")
    os.exit(1)
  end
  if kind == "?" then func() end -- Handle help function
  data = table.filter(data, func)
end

-- Apply sorting
local sort_opt = parser:getopt("sort")
if sort_opt.used then
  local kind = sort_opt.first_arg
  local func = rule.sort[kind]
  if not func then
    print("error: sort '" .. kind .. "' is not defined")
    os.exit(1)
  end
  if kind == "?" then func() end -- Handle help function
...
if not formatter then
  print("error: format '" .. format_name .. "' is not defined")
  os.exit(1)
end
if format_name == "?" then formatter() end -- Handle help function

-- List formatted entries
if parser:getopt("list").used then
